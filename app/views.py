...
class SearchView(APIView):
    def post(self, request):
        ...
        # check if name and email both are empty
        if email == "" and name == "":
            return Response("Enter Search Value", status=404)

        # search by name and email
        elif email != "" and name != "":
            person = Person.objects.filter(Q(email=email) | Q(name__icontains=name))
            serializer = PersonSerializer(person, many=True)
            return Response(serializer.data, status=200)
        # search with name
        elif name != "":
            person = Person.objects.filter(Q(name__icontains=name))
        ...
        # search with email
            ...
            try:
                ...
                return Response("user with this email is not exists", status=404)
